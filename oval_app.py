...
if st.session_state["patient_index"] < len(segmentation_list):
    ...
    if coords1 is not None:
        ...
        with col5:
            ...
            save_path_curve = f'results/curve_plot/{mag_series_uid}.png'
            os.makedirs(os.path.dirname(save_path_curve), exist_ok=True)

            plt.savefig(save_path_curve, bbox_inches = 'tight')

            st.pyplot(fig, width=int(display_width * 1.4))
            plt.close()

            combine_gif_png(gif_path = f'results/segs/{mag_series_uid}.gif', 
                            png_path = save_path_curve, 
            ...
            if st.button("Save Flow  💾", type="primary", key="save_flows", width=display_width):
                ...
                csv_path = f'results/dfs/{mag_series_uid}.csv'
                flow_curve_path = f'results/flow_curves/{mag_series_uid}.npy'
                mask_path = f'results/masks/{mag_series_uid}.nii.gz'
                os.makedirs(os.path.dirname(csv_path), exist_ok=True)
                os.makedirs(os.path.dirname(flow_curve_path), exist_ok=True)
                os.makedirs(os.path.dirname(mask_path), exist_ok=True)

                predicted_df.to_csv(csv_path)
                np.save(flow_curve_path, flow_curve)
