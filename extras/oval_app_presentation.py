...
# -------------------------------
data_path = './data/'
segmentation_list = pd.read_csv('oval_list copy.csv')
...
if st.session_state["patient_index"] < len(segmentation_list):
    ...
    col1, col3, col4 = st.columns([1.5,1.5, 1.5]) # scale the columns so the editor is bigger
    ...
    if coords1 is not None:
        ...
        with col4:
            ...
            # st.write('\n')
            # Save button
            if st.button("Save and Next Scan ➡️", type="primary", key="save_flows", width=display_width):
